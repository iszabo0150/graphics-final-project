...
bool ScenefileReader::parseGroupData(const QJsonObject &object, SceneNode *node) {
    QStringList optionalFields = {"name", "translate", "rotate", "scale", "matrix", "lights", "primitives", "groups", "lsystem"};
    ...
    // parse lsystems if defined !!!!
    if (object.contains("lsystem")) {
        const QJsonValue &lsVal = object["lsystem"];
        if (!lsVal.isObject()) {
            std::cout << "group lsystem must be an object\n";
            return false;
        }
        if (!parseLSystem(lsVal.toObject(), node)) {
            return false;
        }
    }
...
bool ScenefileReader::parsePrimitive(const QJsonObject &prim, SceneNode *node) {
    ...
    QStringList optionalFields = {
        ...
        "blend", "textureFile", "textureU", "textureV", "bumpMapFile", "bumpMapU", "bumpMapV", "bumpMapStrength", "normalMapFile", "normalMapU", "normalMapV", "normalMapStrength" };
    ...
    if (prim.contains("bumpMapFile")) {
        ...
        mat.bumpMap.strength = prim.contains("bumpMapStrength") && prim["bumpMapStrength"].isDouble() ? prim["bumpMapStrength"].toDouble() : 1.0;
    ...
    if (prim.contains("normalMapFile")) {
        ...
        mat.normalMap.strength = prim.contains("normalMapStrength") && prim["normalMapStrength"].isDouble() ? prim["normalMapStrength"].toDouble() : 1.0;
...
bool ScenefileReader::parseLSystem(const QJsonObject &obj, SceneNode *node){
    ...
    if (!obj.contains("axiom") || !obj["axiom"].isString()) {
        std::cout << "lsystem missing or invalid axiom\n";
        return false;
    }
    ls->axiom = obj["axiom"].toString().toStdString();

    // ITERATIONS
    if (obj.contains("iterations")) {
        if (!obj["iterations"].isDouble()) {
            std::cout << "lsystem iterations must be numeric\n";
            return false;
        }
        ls->iterations = obj["iterations"].toInt();
    }

    // ANGLE
    if (obj.contains("angle")) {
        if (!obj["angle"].isDouble()) {
            std::cout << "lsystem angle must be numeric\n";
            return false;
        }
        float deg = obj["angle"].toDouble();
        ls->angle = glm::radians(deg);
    }

    // STEP LENGTH
    if (obj.contains("step")) {
        if (!obj["step"].isDouble()) {
            std::cout << "lsystem step must be numeric\n";
            return false;
        }
        ls->step = obj["step"].toDouble();
    }

    // STEM PRIMITIVE
    if (obj.contains("stemPrimitive")) {
        if (!obj["stemPrimitive"].isString()) {
            std::cout << "lsystem stemPrimitive must be string\n";
            return false;
        }
        std::string s = obj["stemPrimitive"].toString().toStdString();

        if (s == "cylinder") ls->stemPrimitive = PrimitiveType::PRIMITIVE_CYLINDER;
        else if (s == "cone") ls->stemPrimitive = PrimitiveType::PRIMITIVE_CONE;
        else if (s == "cube") ls->stemPrimitive = PrimitiveType::PRIMITIVE_CUBE;
        else if (s == "sphere") ls->stemPrimitive = PrimitiveType::PRIMITIVE_SPHERE;
        else {
            std::cout << "unknown stemPrimitive: " << s << "\n";
            return false;
        }
    }

    // LEAF PRIMITIVE
    if (obj.contains("leafPrimitive")) {
        if (!obj["leafPrimitive"].isString()) {
            std::cout << "lsystem leafPrimitive must be string\n";
            return false;
        }
        std::string s = obj["leafPrimitive"].toString().toStdString();

        if (s == "cube") ls->leafPrimitive = PrimitiveType::PRIMITIVE_CUBE;
        else if (s == "sphere") ls->leafPrimitive = PrimitiveType::PRIMITIVE_SPHERE;
        else if (s == "cone") ls->leafPrimitive = PrimitiveType::PRIMITIVE_CONE;
        else if (s == "cylinder") ls->leafPrimitive = PrimitiveType::PRIMITIVE_CYLINDER;
        else {
            std::cout << "unknown leafPrimitive: " << s << "\n";
            return false;
        }
    }

    // STEM MATERIAL
    if (obj.contains("stemMaterial")) {
        if (!obj["stemMaterial"].isObject()) {
            std::cout << "lsystem stemMaterial must be an object\n";
            return false;
        }
        // Actually parse the stem material!
        ls->stemMaterial.clear();
        if (!parseMaterialProperties(obj["stemMaterial"].toObject(), ls->stemMaterial)) {
            return false;
        }
    } else {
        // Default stem material (brown)
        ls->stemMaterial.clear();
        ls->stemMaterial.cDiffuse = glm::vec4(0.55f, 0.27f, 0.07f, 1.0f);
        ls->stemMaterial.cAmbient = glm::vec4(0.2f, 0.1f, 0.02f, 1.0f);
        ls->stemMaterial.cSpecular = glm::vec4(0.1f, 0.1f, 0.1f, 1.0f);
        ls->stemMaterial.shininess = 5.0f;
    }

    // After the STEM MATERIAL section
    std::cout << "=== AFTER PARSING STEM MATERIAL ===" << std::endl;
    std::cout << "Stem diffuse: " << ls->stemMaterial.cDiffuse.r << ", "
              << ls->stemMaterial.cDiffuse.g << ", "
    ...
    if (!obj.contains("rules") || !obj["rules"].isArray()) {
        std::cout << "lsystem missing rules array\n";
        return false;
    }

    QJsonArray arr = obj["rules"].toArray();
    for (const QJsonValue &rv : arr) {
        if (!rv.isObject()) {
            std::cout << "lsystem rule must be object\n";
            return false;
        }
        ...
        // INPUT
        if (!r.contains("input") || !r["input"].isString()) {
            std::cout << "lsystem rule missing input\n";
            return false;
        }
        rule.input = r["input"].toString().toStdString();

        // PARAMS
        if (r.contains("params")) {
            if (!r["params"].isArray()) {
                std::cout << "lsystem rule params must be array\n";
                return false;
            }
            for (auto p : r["params"].toArray()) {
                if (!p.isString()) {
                    std::cout << "lsystem param must be string\n";
                    return false;
                }
                rule.params.push_back(p.toString().toStdString());
            }
        }

        // CONDITION
        if (r.contains("condition")) {
            if (!r["condition"].isString()) {
                std::cout << "lsystem rule condition must be string\n";
                return false;
            }
            rule.condition = r["condition"].toString().toStdString();
        }

        // OUTPUT
        if (!r.contains("output") || !r["output"].isString()) {
            std::cout << "lsystem rule missing output\n";
            return false;
        }
        rule.output = r["output"].toString().toStdString();

        // PROBABILITY
        if (r.contains("probability")) {
            if (!r["probability"].isDouble()) {
                std::cout << "lsystem rule probability must be numeric\n";
                return false;
            }
            rule.probability = r["probability"].toDouble();
        }

        ls->rules.push_back(rule);
    }

    return true;
}

// Add this helper function to parse material properties
bool ScenefileReader::parseMaterialProperties(const QJsonObject &matObj, SceneMaterial &mat) {
    if (matObj.contains("ambient")) {
        if (!matObj["ambient"].isArray()) {
            std::cout << "material ambient must be of type array" << std::endl;
            return false;
        }
        QJsonArray ambientArray = matObj["ambient"].toArray();
        if (ambientArray.size() != 3) {
            std::cout << "material ambient array must be of size 3" << std::endl;
            return false;
        }
        for (int i = 0; i < 3; i++) {
            if (!ambientArray[i].isDouble()) {
                std::cout << "material ambient must contain floating-point values" << std::endl;
                return false;
            }
            mat.cAmbient[i] = ambientArray[i].toDouble();
        }
    ...
    if (matObj.contains("diffuse")) {
        if (!matObj["diffuse"].isArray()) {
            std::cout << "material diffuse must be of type array" << std::endl;
            return false;
        }
        QJsonArray diffuseArray = matObj["diffuse"].toArray();
        if (diffuseArray.size() != 3) {
            std::cout << "material diffuse array must be of size 3" << std::endl;
            return false;
        }
        for (int i = 0; i < 3; i++) {
            if (!diffuseArray[i].isDouble()) {
                std::cout << "material diffuse must contain floating-point values" << std::endl;
                return false;
            }
            mat.cDiffuse[i] = diffuseArray[i].toDouble();
        }
    ...
    if (matObj.contains("specular")) {
        if (!matObj["specular"].isArray()) {
            std::cout << "material specular must be of type array" << std::endl;
            return false;
        }
        QJsonArray specularArray = matObj["specular"].toArray();
        if (specularArray.size() != 3) {
            std::cout << "material specular array must be of size 3" << std::endl;
            return false;
        }
        for (int i = 0; i < 3; i++) {
            if (!specularArray[i].isDouble()) {
                std::cout << "material specular must contain floating-point values" << std::endl;
                return false;
            }
            mat.cSpecular[i] = specularArray[i].toDouble();
        }
    ...
    if (matObj.contains("reflective")) {
        if (!matObj["reflective"].isArray()) {
            std::cout << "material reflective must be of type array" << std::endl;
            return false;
        }
        QJsonArray reflectiveArray = matObj["reflective"].toArray();
        if (reflectiveArray.size() != 3) {
            std::cout << "material reflective array must be of size 3" << std::endl;
            return false;
        }
        for (int i = 0; i < 3; i++) {
            if (!reflectiveArray[i].isDouble()) {
                std::cout << "material reflective must contain floating-point values" << std::endl;
                return false;
            }
            mat.cReflective[i] = reflectiveArray[i].toDouble();
        }
    ...
    if (matObj.contains("transparent")) {
        if (!matObj["transparent"].isArray()) {
            std::cout << "material transparent must be of type array" << std::endl;
            return false;
        }
        QJsonArray transparentArray = matObj["transparent"].toArray();
        if (transparentArray.size() != 3) {
            std::cout << "material transparent array must be of size 3" << std::endl;
            return false;
        }
        for (int i = 0; i < 3; i++) {
            if (!transparentArray[i].isDouble()) {
                std::cout << "material transparent must contain floating-point values" << std::endl;
                return false;
            }
            mat.cTransparent[i] = transparentArray[i].toDouble();
        }
    ...
    if (matObj.contains("shininess")) {
        if (!matObj["shininess"].isDouble()) {
            std::cout << "material shininess must be of type float" << std::endl;
            return false;
        }
        mat.shininess = (float)matObj["shininess"].toDouble();
    }

    if (matObj.contains("ior")) {
        if (!matObj["ior"].isDouble()) {
            std::cout << "material ior must be of type float" << std::endl;
            return false;
        }
        mat.ior = (float)matObj["ior"].toDouble();
    }

    if (matObj.contains("blend")) {
        if (!matObj["blend"].isDouble()) {
            std::cout << "material blend must be of type float" << std::endl;
            return false;
        }
