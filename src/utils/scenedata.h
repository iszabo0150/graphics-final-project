...
struct SceneFileMap {
    ...
    float repeatU;
    float repeatV;
    float strength = 1; //used for normal / bump mapping
...
struct LSystemRule {
    std::string input; // "A"
    std::vector<std::string> params; // ("s")
    std::string condition; // "s > 0.3"
    std::string output; // "F(s)..."
    float probability = 1.0f; // for stochastic rules ! so like if there's only a 0.3 percent that this rule happens
};
...
struct LSystemData {
    ...
    std::string axiom;                 // "A(1.0)"
    int iterations = 4;

    // Base angle & step length for turtle
    float angle = glm::radians(25.f);   // angle in radians :P
    float step = 1.0f;

    // primitive types for stems/leaves
    PrimitiveType stemPrimitive = PrimitiveType::PRIMITIVE_CYLINDER;
